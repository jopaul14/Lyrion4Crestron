...
**Status:** Authoritative. This document supersedes the original refactor specification (formerly in `CLAUDE.md`) and describes the system **as built** at driver version 1.0.8. Where this document and older documents disagree, this document wins.
...
- **Power re-assert: an explicit command always reaches the platform, even when nothing changes.** Power reaches Crestron Home as an *edge*, and that edge must survive three change-gates in series (LMS notifies only on a real transition; the registry raises `PowerStateChanged` only when its stored value changes; the Source/Receiver emit only when their own value changes). Crestron Home's room state and the player's power state drift apart routinely — a room turned off without the LMS player powering off, or a bind-time emit landing during driver load before Crestron Home's Actions & Events engine is listening — and once drifted, an explicit power-on changes nothing anywhere, so `Room On` never runs and no mechanism re-syncs them. Therefore: when `PowerOn`/`PowerOff` finds the registry already holding the requested state, the registry raises `PowerStateReasserted`, and consumers that publish power as an edge (Source, Receiver) re-emit past their own change-gate. This is **not** a registry mutation and does not weaken the change-gate invariant — it fires only on a deliberate power command, never from playback, a status refresh, or a reconnect, and is rate-limited to one per player per five seconds so the resulting `Room On` cannot echo back into another re-assert. Consumers that publish power as a *level* (the Helper's `Power` property) correctly ignore it. Note the corollary: **the Helper looking correct is not evidence that an edge was published**, because a level reads right whenever the value is right, including when nothing was raised at all.
- **Availability restore re-syncs, it does not just reconnect.** Losing a player drives local power/playback to off/stopped in the Source and Helper; the registry never sees those writes, so a player that stayed powered on through the dropout produces no edge to undo them. Consumers re-pull the registry snapshot and force the emits on availability restore. `RepublishAll` covers a *server* reconnect only — a per-player `client disconnect`/`reconnect` flap never reaches it.
...
- **One narrow exception, added in 1.0.8: the Source logs a power *re-assert*.** This is not a power-state change — the state did not change; it is the record of a detected *disagreement* between Crestron Home's room state and the player's power state, which is exactly the condition that made "Power Is On → Room On" silently stop working before 1.0.8. It cannot fire from playback, a status refresh, a reconnect, or any derived power change: only from an explicit `PowerOn`/`PowerOff` that found the requested state already held, and then at most once per player per five seconds. The Receiver deliberately does not log the same event, so one re-assert produces one line. If a future change makes re-asserts frequent, that is a bug in the re-assert condition, not a reason to widen the logging rule.
