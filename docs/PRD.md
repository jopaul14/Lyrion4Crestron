...
**Status:** Authoritative. This document supersedes the original refactor specification (formerly in `CLAUDE.md`) and describes the system **as built** at driver version 1.0.11. Where this document and older documents disagree, this document wins.
...
- **A bind-time snapshot is published only if it was observed; the first real observation always publishes.** When a consumer binds, it applies the registry's snapshot to Crestron Home. That snapshot is *forced* past the consumer's change-gate only when the record is available — which requires a status response to have arrived — because that is the reload case where the registry holds real state that must reach Crestron Home even where it equals the framework default. On a cold boot the record is a blank default, and forcing it would assert "powered off" for a player nobody has looked at; with a "Power Is Off → Room Off" mapping, Crestron Home turns that into a real power-off (seen 2026-09-02: two players playing through a processor reboot, one killed every time). The other half of the contract lives in the registry: the first explicit power report for a record publishes even when its value equals the default (`HasExplicitPower` false→true is the change), so a consumer whose copy went stale across a Lyrion Server driver reload is synced at first sight. **General rule: never force-publish a value the Lyrion Server has not observed.** 1.0.8 broke this on availability-restore; 1.0.11 fixed the same class of fault at bind.
